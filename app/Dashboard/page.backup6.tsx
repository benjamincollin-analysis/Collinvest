                  ...
                  <div style={{ width: "100%", marginBottom: "8px" }}>
                    <ResponsiveContainer width="100%" height={260}>
                      <AreaChart data={labels.map((l, i) => ({
                        ...
                        {/* Property milestone reference lines with staggered labels */}
                        {allMilestones.map((m, i) => (
                          <ReferenceLine key={i} x={labels[m.idx]} stroke={`${m.color}50`} strokeDasharray="3 2" label={{ value: m.label, fill: m.color, fontSize: 11, fontWeight: "bold", position: "insideTopLeft", offset: i % 2 === 0 ? 4 : 22 }}/>
                        ))}
                        ...
                        <Area type="monotone" dataKey="portfolio" stroke="#f59e0b" strokeWidth={3} fill="url(#portfolioGrad)" dot={(props: any) => {
                          ...
                          if (!milestone) return <circle key={index} cx={cx} cy={cy} r={3} fill="#f59e0b" opacity={0.5}/>;
                          return <g key={index}>
